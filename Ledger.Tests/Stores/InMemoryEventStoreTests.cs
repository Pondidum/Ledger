...
namespace Ledger.Tests.Stores
{
	public class InMemoryEventStoreTests
	{
		...
		public void When_getting_events_they_are_ordered()
		{
			...
			using (var writer = store.CreateWriter<int>())
			{
				writer.SaveEvents(null, 1, new List<IDomainEvent>() {e0});
				writer.SaveEvents(null, 2, new List<IDomainEvent>() {e1});
				writer.SaveEvents(null, 1, new List<IDomainEvent>() {e2});
			}
		...
		public void When_getting_snapshots_they_are_ordered()
		{
			...
			using (var writer = store.CreateWriter<int>())
			{
				writer.SaveSnapshot(null, 1, snap0);
				writer.SaveSnapshot(null, 2, snap1);
				writer.SaveSnapshot(null, 1, snap2);
			}
